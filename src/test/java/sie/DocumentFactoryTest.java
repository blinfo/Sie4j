...
public class DocumentFactoryTest {
    ...
    public void test_file_with_missing_program_version() {
        ...
        assertTrue(factory.getLogs().stream().map(l -> l.getMessage()).anyMatch(m -> m.equals(message)));
    }
    ...
    public void file_with_long_voucher_series_numbers_should_add_info() {
        ...
        long count = factory.getDocument().vouchers()
                .stream()
                .filter(voucher -> voucher.optSeries().orElse("").length() > 1)
                .count();
    ...
    public void test_file_with_erroneous_voucher_date() {
        ...
        assertEquals(expectedMessage, factory.getLogs().get(0).getMessage());
    }
